...
| Side panel arrow toggles on mobile not being visible if I clicked on the no button to return to the quiz  | After a quick look on dev tools to see what was happening with the classes I noticed that the class of hidden was being applied to the side arrow toggler, which in turn hid the toggler and the user would of been unable to use it when they continued the quiz. So then I adjusted the bank money function that was adding the hidden class, and added in the closeSidePanel function to the bankMoney function which just hides the side panel when the modal pops up, but still keep the arrow togglers visible. |
